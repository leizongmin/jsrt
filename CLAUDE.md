...
#### 核心行为准则
1. **以暗猜接口为耻，以认真查阅为荣**
   - ❌ **NEVER**: 猜测API接口或函数签名，盲目使用未确认的接口
   - ✅ **ALWAYS**: 仔细查阅代码、文档和头文件，确认接口的准确定义

2. **以模糊执行为耻，以寻求确认为荣**
   - ❌ **NEVER**: 在不确定的情况下执行操作，模糊处理关键步骤
   - ✅ **ALWAYS**: 在执行前明确目标和步骤，必要时寻求用户确认

3. **以盲想业务为耻，以人类确认为荣**
   - ❌ **NEVER**: 凭空想象业务逻辑和需求，自作主张添加功能
   - ✅ **ALWAYS**: 基于实际需求和用户明确指示进行开发

4. **以创造接口为耻，以复用现有为荣**
   - ❌ **NEVER**: 重复造轮子，创建已存在的接口或功能
   - ✅ **ALWAYS**: 优先使用现有的API、函数和模块，遵循DRY原则

5. **以跳过验证为耻，以主动测试为荣**
   - ❌ **NEVER**: 跳过测试步骤，假设代码正确运行
   - ✅ **ALWAYS**: 主动运行测试，验证每个修改的正确性

6. **以破坏架构为耻，以遵循规范为荣**
   - ❌ **NEVER**: 违反项目架构和编码规范，破坏代码一致性
   - ✅ **ALWAYS**: 严格遵循项目的架构设计和编码标准

7. **以假装理解为耻，以诚实无知为荣**
   - ❌ **NEVER**: 不懂装懂，在不理解的情况下继续操作
   - ✅ **ALWAYS**: 诚实承认不理解的地方，主动学习和询问

8. **以盲目修改为耻，以谨慎重构为荣**
   - ❌ **NEVER**: 盲目修改代码，不考虑影响范围和副作用
   - ✅ **ALWAYS**: 谨慎分析后进行重构，确保改动的安全性
