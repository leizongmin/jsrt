...
static JSValue JSRT_CloneObject(JSContext *ctx, JSValue object, JSRT_CloneMap *map) {
  ...
  if (JS_GetOwnPropertyNames(ctx, &props, &prop_count, object, JS_GPN_STRING_MASK | JS_GPN_SYMBOL_MASK | JS_GPN_ENUM_ONLY) < 0) {
...
void JSRT_RuntimeSetupStdClone(JSRT_Runtime *rt) {
  ...
  JS_SetPropertyStr(ctx, rt->global, "structuredClone", JS_NewCFunction(ctx, JSRT_StructuredClone, "structuredClone", 1));
