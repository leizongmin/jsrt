...
static int jsrt_static_digest_data(jsrt_crypto_algorithm_t alg, const uint8_t* input, size_t input_len, uint8_t** output, size_t* output_len) {
...
static void jsrt_digest_work(uv_work_t* req) {
  ...
  int result = jsrt_static_digest_data(op->algorithm, op->input_data, op->input_length, &op->output_data, &op->output_length);
...
static void jsrt_digest_after_work(uv_work_t* req, int status) {
  ...
  if (op->error_code != 0 || status != 0) {
    ...
    JS_SetPropertyStr(ctx, error, "message", JS_NewString(ctx, op->error_message ? op->error_message : "Unknown error"));
...
jsrt_crypto_algorithm_t jsrt_crypto_parse_algorithm(JSContext* ctx, JSValue algorithm) {
  if (JS_IsString(algorithm)) {
    ...
    if (strcmp(alg_name, "SHA-1") == 0) result = JSRT_CRYPTO_ALG_SHA1;
    else if (strcmp(alg_name, "SHA-256") == 0) result = JSRT_CRYPTO_ALG_SHA256;
    else if (strcmp(alg_name, "SHA-384") == 0) result = JSRT_CRYPTO_ALG_SHA384;
    else if (strcmp(alg_name, "SHA-512") == 0) result = JSRT_CRYPTO_ALG_SHA512;
...
const char* jsrt_crypto_algorithm_to_string(jsrt_crypto_algorithm_t alg) {
  switch (alg) {
    case JSRT_CRYPTO_ALG_SHA1: return "SHA-1";
    case JSRT_CRYPTO_ALG_SHA256: return "SHA-256";
    case JSRT_CRYPTO_ALG_SHA384: return "SHA-384";
    case JSRT_CRYPTO_ALG_SHA512: return "SHA-512";
