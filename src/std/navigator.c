...
static JSValue JSRT_NavigatorGetUserAgent(JSContext* ctx, JSValueConst this_val, int argc, JSValueConst* argv) {
  ...
  size_t ua_len =
      strlen("jsrt/") + strlen(jsrt_version) + strlen(" (JavaScript Runtime) QuickJS/") + strlen(quickjs_version) + 1;
  ...
  snprintf(user_agent, ua_len, "jsrt/%s (JavaScript Runtime) QuickJS/%s", jsrt_version, quickjs_version);
...
JSValue jsrt_init_module_navigator(JSContext* ctx) {
  ...
  JS_DefinePropertyGetSet(ctx, navigator_obj, JS_NewAtom(ctx, "userAgent"),
                          JS_NewCFunction(ctx, JSRT_NavigatorGetUserAgent, "get userAgent", 0), JS_UNDEFINED,
