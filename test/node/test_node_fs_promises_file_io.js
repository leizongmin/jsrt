...
async function runTests() {
  ...
  await fsPromises.writeFile(
    path.join(tmpdir, 'test_promises_write.txt'),
    'Hello World'
  );
  ...
  await fsPromises.writeFile(
    path.join(tmpdir, 'test_promises_binary.txt'),
    buf
  );
  ...
  await fsPromises
    .unlink(path.join(tmpdir, 'test_promises_append_new.txt'))
    .catch(() => {});
  await fsPromises.appendFile(
    path.join(tmpdir, 'test_promises_append_new.txt'),
    'New file'
  );
  const data5 = await fsPromises.readFile(
    path.join(tmpdir, 'test_promises_append_new.txt')
  );
