...
assert.strictEqual(
  ...
  true,
  'Date object should return true'
);
assert.strictEqual(
  util.types.isDate({}),
  false,
  'Plain object should return false'
);
assert.strictEqual(
  util.types.isDate('2025-01-01'),
  false,
  'String should return false'
);
assert.strictEqual(util.types.isDate(null), false, 'null should return false');
assert.strictEqual(
  util.types.isDate(undefined),
  false,
...
assert.strictEqual(
  ...
  true,
  'Promise should return true'
);
assert.strictEqual(
  util.types.isPromise(Promise.reject().catch(() => {})),
  true,
  'Rejected promise should return true'
);
assert.strictEqual(
...
assert.strictEqual(
  util.types.isPromise({ then: () => {} }),
  false,
  'Thenable should return false'
);
assert.strictEqual(
  util.types.isPromise(null),
  false,
...
assert.strictEqual(
  ...
  true,
  'RegExp literal should return true'
);
assert.strictEqual(
  util.types.isRegExp(new RegExp('abc')),
  true,
  'RegExp constructor should return true'
);
assert.strictEqual(
  util.types.isRegExp(/test/gi),
  true,
  'RegExp with flags should return true'
);
assert.strictEqual(
  util.types.isRegExp('abc'),
  false,
...
assert.strictEqual(
  util.types.isArrayBuffer(new ArrayBuffer(10)),
  true,
  'ArrayBuffer should return true'
);
assert.strictEqual(
  util.types.isArrayBuffer(new Uint8Array(10)),
  false,
  'TypedArray should return false'
);
assert.strictEqual(
...
assert.strictEqual(
  typeof util.types,
  'object',
  'util.types should be an object'
);
assert.strictEqual(
  typeof util.types.isDate,
  'function',
  'isDate should be a function'
);
assert.strictEqual(
  typeof util.types.isPromise,
  'function',
  'isPromise should be a function'
);
assert.strictEqual(
  typeof util.types.isRegExp,
  'function',
  'isRegExp should be a function'
);
assert.strictEqual(
  typeof util.types.isArrayBuffer,
  'function',
  'isArrayBuffer should be a function'
);
