...
for (const testCase of sizes) {
  ...
  console.log(`  gzipSync: ${gzipResult.perOp.toFixed(2)}ms/op (total: ${gzipResult.elapsed}ms)`);
  ...
  console.log(`  gunzipSync: ${gunzipResult.perOp.toFixed(2)}ms/op (total: ${gunzipResult.elapsed}ms)`);
  ...
  console.log(`  deflateSync: ${deflateResult.perOp.toFixed(2)}ms/op (total: ${deflateResult.elapsed}ms)`);
  
  // Benchmark inflateSync
  const inflateResult = benchmark('inflateSync', () => {
    zlib.inflateSync(deflated);
  });
  console.log(`  inflateSync: ${inflateResult.perOp.toFixed(2)}ms/op (total: ${inflateResult.elapsed}ms)`);
  ...
  console.log(`    Level 1: ${level1Result.perOp.toFixed(2)}ms/op, size: ${level1.length} bytes`);
  console.log(`    Level 9: ${level9Result.perOp.toFixed(2)}ms/op, size: ${level9.length} bytes`);
  console.log(`    Speed difference: ${((level9Result.perOp / level1Result.perOp - 1) * 100).toFixed(1)}% slower`);
  console.log(`    Size difference: ${((1 - level9.length / level1.length) * 100).toFixed(1)}% smaller`);
...
console.log('\n' + '='.repeat(50));
console.log('Summary:\n');
console.log('Size     | gzipSync | gunzipSync | deflateSync | inflateSync | Ratio');
...
if (results.length > 0) {
  ...
  const gunzipThroughput = (largest.bytes / 1024 / 1024) / (largest.gunzip / 1000);
