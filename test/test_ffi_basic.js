...
try {
  ...
  console.log(
    'Available functions:',
    Object.keys(libc).filter((key) => typeof libc[key] === 'function')
  );
  ...
  try {
    console.log(
      'abs function properties:',
      Object.getOwnPropertyNames(libc.abs)
    );
  ...
  console.log(
    '⚠️ libc loading failed (this is expected on some systems):',
    error.message
  );
}
...
try {
  ...
  console.log(
    '✓ Correctly threw error for non-existent library:',
    error.message
  );
  assert.strictEqual(
    error.name,
