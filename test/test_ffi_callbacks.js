...
try {
  ...
  assert.strictEqual(typeof callbackPtr, 'number', 'Callback should return a pointer');
  ...
  const multiCallbackPtr = ffi.Callback(multiArgCallback, 'int', ['int', 'int']);
  assert.strictEqual(typeof multiCallbackPtr, 'number', 'Multi-arg callback should return a pointer');
  ...
  assert.strictEqual(typeof stringCallbackPtr, 'number', 'String callback should return a pointer');
  ...
  assert.strictEqual(typeof voidCallbackPtr, 'number', 'Void callback should return a pointer');
  ...
  try {
    ...
    assert.strictEqual(error.message.includes('First argument must be a JavaScript function'), true);
  ...
  assert.strictEqual(ffi.types.callback, 'callback', 'Callback type should be available');
