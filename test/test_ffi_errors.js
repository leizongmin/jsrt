...
try {
  ...
  assert(
    ...
    'Error should specify expected arguments'
  );
}
...
try {
  ...
  console.log(
    '✓ Enhanced error for invalid function definitions:',
    error.message
  );
  assert(
    error.message.includes('FFI Error in ffi.Library'),
...
try {
  ...
  assert(
    error.message.includes('Cannot allocate zero bytes'),
...
try {
  ...
  assert(
    ...
    'Error should explain size limit'
  );
  assert(
    error.message.includes('maximum: 1GB'),
    'Error should specify maximum size'
  );
}
...
try {
  ...
  console.log(
    '✓ Enhanced error for arrayFromPointer no arguments:',
    error.message
  );
  assert(
...
try {
  ...
  console.log(
    '✓ Enhanced error for arrayFromPointer invalid type:',
    error.message
  );
  assert(
    error.message.includes('FFI Error in ffi.arrayFromPointer'),
  ...
  assert(
    error.message.includes('Invalid array element type'),
    'Error should explain the issue'
  );
  assert(
    error.message.includes('invalid_type'),
    'Error should show the invalid type'
  );
}
...
try {
  ...
  console.log(
    '✓ Enhanced error for arrayFromPointer oversized array:',
    error.message
  );
  assert(
    error.message.includes('FFI Error in ffi.arrayFromPointer'),
  ...
  assert(
    error.message.includes('Array length too large'),
    'Error should explain the issue'
  );
  assert(
    error.message.includes('maximum: 1M elements'),
    'Error should specify maximum size'
  );
}
...
try {
  ...
  assert(
    error.message.includes('non_existent_library_xyz.so'),
    'Error should include library name'
  );
  assert(
  ...
  console.log(
    'Library loading error (truncated):',
    error.message.substring(0, 200) + '...'
  );
  console.log(
    '✓ Enhanced error includes helpful suggestions and platform-specific advice'
  );
}
