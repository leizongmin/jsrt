...
const testContent = 'Hello World! This is a test file with some content for truncation testing.';
...
try {
    ...
    assert.strictEqual(originalStats.size, testContent.length, 'Original file should have expected size');
    ...
    assert.strictEqual(truncatedStats.size, 10, 'File should be truncated to 10 bytes');
    
    // Verify content is truncated correctly
    const truncatedContent = fs.readFileSync(testFile, 'utf8');
    assert.strictEqual(truncatedContent, testContent.substring(0, 10), 'Content should be truncated correctly');
    
    // Test truncating to 0 (default)
    fs.truncateSync(testFile);
    const emptyStats = fs.statSync(testFile);
    assert.strictEqual(emptyStats.size, 0, 'File should be truncated to 0 bytes when no length specified');
...
try {
    ...
    try {
        ...
        // Verify truncated size
        const truncatedStats = fs.statSync(testFile);
        assert.strictEqual(truncatedStats.size, 15, 'File should be truncated to 15 bytes using fd');
        
        // Verify content
        const truncatedContent = fs.readFileSync(testFile, 'utf8');
        assert.strictEqual(truncatedContent, testContent.substring(0, 15), 'Content should be truncated correctly using fd');
        
        // Test default truncation to 0
        fs.ftruncateSync(fd);
        const emptyStats = fs.statSync(testFile);
        assert.strictEqual(emptyStats.size, 0, 'File should be truncated to 0 bytes using fd when no length specified');
...
try {
    ...
    assert.strictEqual(fs.existsSync(tempDir), true, 'Temporary directory should exist');
    ...
    assert.strictEqual(tempDir.startsWith(tempPrefix), true, 'Temporary directory should start with prefix');
    
    // Verify the directory name is longer than the prefix (has random suffix)
    assert.strictEqual(tempDir.length > tempPrefix.length, true, 'Temporary directory should have random suffix');
    
    // Test that multiple calls create different directories
    const tempDir2 = fs.mkdtempSync(tempPrefix);
    assert.notStrictEqual(tempDir, tempDir2, 'Multiple mkdtemp calls should create different directories');
...
try {
    ...
    assert.strictEqual(typeof tempDir1, 'string', 'mkdtempSync with utf8 should return string');
    
    // Test with options object
    const tempDir2 = fs.mkdtempSync(tempPrefix, { encoding: 'utf8' });
    assert.strictEqual(typeof tempDir2, 'string', 'mkdtempSync with options object should return string');
    
    // Test with buffer encoding
    const tempDirBuffer = fs.mkdtempSync(tempPrefix, { encoding: 'buffer' });
    assert.strictEqual(Buffer.isBuffer(tempDirBuffer), true, 'mkdtempSync with buffer encoding should return Buffer');
...
try {
    ...
    try {
        ...
        assert.strictEqual(error.code, 'ENOENT', 'truncateSync should throw ENOENT error');
        assert.strictEqual(error.syscall, 'truncate', 'Error should have correct syscall');
    ...
    try {
        ...
        assert.strictEqual(typeof error.code === 'string', true, 'ftruncateSync should throw error with code');
    ...
    try {
        ...
        assert.strictEqual(error.code, 'ENOENT', 'mkdtempSync should throw ENOENT error');
...
try {
    ...
    try {
        ...
        assert.strictEqual(error.name, 'RangeError', 'truncateSync should throw RangeError for negative length');
    ...
    try {
        try {
            ...
            assert.strictEqual(error.name, 'RangeError', 'ftruncateSync should throw RangeError for negative length');
