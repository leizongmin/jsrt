...
async function runTests() {
  ...
  await fsPromises.writeFile('/tmp/test_promises_write.txt', 'Hello World');
  console.log('  ✓ File written successfully');

  console.log('Test 2: fsPromises.readFile() - read back string');
  const data = await fsPromises.readFile('/tmp/test_promises_write.txt');
  const text = Buffer.from(data).toString('utf8');
  console.log(`  Read: "${text}"`);
  assert(text === 'Hello World', 'content should match');

  console.log('Test 3: fsPromises.appendFile() - append text');
  await fsPromises.appendFile('/tmp/test_promises_write.txt', ' - Appended');
  const data2 = await fsPromises.readFile('/tmp/test_promises_write.txt');
  ...
  await fsPromises.writeFile('/tmp/test_promises_binary.txt', buf);
  const data3 = await fsPromises.readFile('/tmp/test_promises_binary.txt');
  ...
  try {
    await fsPromises.readFile('/tmp/nonexistent_file.txt');
  ...
  await fsPromises.writeFile('/tmp/test_promises_overwrite.txt', 'First write');
  await fsPromises.writeFile(
    '/tmp/test_promises_overwrite.txt',
    'Second write'
  );
  const data4 = await fsPromises.readFile('/tmp/test_promises_overwrite.txt');
  ...
  await fsPromises.unlink('/tmp/test_promises_append_new.txt').catch(() => {});
  await fsPromises.appendFile('/tmp/test_promises_append_new.txt', 'New file');
  const data5 = await fsPromises.readFile('/tmp/test_promises_append_new.txt');
  const text5 = Buffer.from(data5).toString('utf8');
  assert(text5 === 'New file', 'append should create new file');

  // Cleanup
  await fsPromises.unlink('/tmp/test_promises_write.txt');
  await fsPromises.unlink('/tmp/test_promises_binary.txt');
  await fsPromises.unlink('/tmp/test_promises_overwrite.txt');
  await fsPromises.unlink('/tmp/test_promises_append_new.txt');
