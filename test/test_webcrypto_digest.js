  ...
  try {
    ...
    if (result && typeof result.then === 'function') {
      ...
      result.then(hash => {
        ...
        console.log('Hash (first 8 bytes):', Array.from(hashArray.slice(0, 8)));
        
        // Expected SHA-256 of "hello": 2cf24dba4f21d4288dfcb1eb908e6ba18a40d8e14a03c5cdf2df5659cfd4d2cf
        const expected_start = [0x2c, 0xf2, 0x4d, 0xba, 0x4f, 0x21, 0xd4, 0x28];
  ...
  algorithms.forEach((alg, index) => {
    ...
    try {
      ...
      if (result && typeof result.then === 'function') {
        result.then(hash => {
          const hashArray = new Uint8Array(hash);
          console.log(`✅ PASS: ${alg} digest successful (${hashArray.length} bytes)`);
          
          if (index === algorithms.length - 1) {
            console.log('=== WebCrypto Digest Tests Completed Successfully ===');
          }
        }).catch(error => {
      ...
      console.error(`❌ FAIL: ${alg} threw exception:`, error.name, error.message);
    }
  });
}