...
function testMixedAlgorithmUsages() {
  ...
  return crypto.subtle.generateKey(
    ...
    console.log('⚠️ Implementation allows mixed ECDSA/ECDH usages (flexible but non-standard)');
    ...
    console.log('✅ Correctly rejected mixed ECDSA/ECDH usages (strict implementation)');
  });
}
...
function testSameCurveCompatibility() {
  ...
  return Promise.all([ecdsaPromise, ecdhPromise]).then(function(keyPairs) {
    ...
    assert.strictEqual(ecdsaKeys.publicKey.algorithm.namedCurve, 'P-256', 'ECDSA curve should be P-256');
    assert.strictEqual(ecdhKeys.publicKey.algorithm.namedCurve, 'P-256', 'ECDH curve should be P-256');
    ...
    return crypto.subtle.deriveBits(
      ...
      console.log('⚠️ Allowed using ECDSA public key for ECDH (implementation dependent)');
    }).catch(function(error) {
      console.log('✅ Correctly rejected using ECDSA key for ECDH:', error.message);
    });
  }).catch(function(error) {
    console.error('❌ Same curve compatibility test failed:', error.message);
    throw error;
  });
}
...
function testCombinedWorkflow() {
  ...
  return Promise.all([aliceECDSA, aliceECDH, bobECDSA, bobECDH]).then(function(keys) {
    ...
    const message = new TextEncoder().encode("Authenticated message from Alice");
    ...
    return Promise.all([sharedSecretPromise, signaturePromise]).then(function(results) {
      const sharedSecret = new Uint8Array(results[0]);
      const signature = results[1];
      
      console.log('✅ Derived shared secret:', sharedSecret.length, 'bytes');
      ...
      console.log('✅ Combined ECDH key exchange and ECDSA signature workflow succeeded');
    });
  }).catch(function(error) {
...
function testAllCurveCombinations() {
  ...
  curves.forEach(function(curve) {
    ...
    const ecdhPromise = Promise.all([
      ...
      assert.strictEqual(secret.byteLength, 32, 'ECDH should produce 32 bytes');
...
function testPerformanceComparison() {
  ...
  function measureCurvePerformance(curve) {
    ...
    return crypto.subtle.generateKey(
      ...
      console.log('  ', curve, ':', elapsed.toFixed(2), 'ms for', iterations, 'sign/verify operations');
      return elapsed;
    });
  }
  ...
  return Promise.all(performancePromises).then(function(times) {
    console.log('✅ Performance comparison completed');
    console.log('   Note: Larger curves generally take more time but provide more security');
  }).catch(function(error) {
...
function testKeyReuse() {
  ...
  return crypto.subtle.generateKey(
    ...
    results.forEach(function(result) {
      assert.strictEqual(result.valid, true, 'Message "' + result.message + '" should verify');
    });
    console.log('✅ Same key pair successfully reused for', results.length, 'operations');
  }).catch(function(error) {
