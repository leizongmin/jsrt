...
function testECDHKeyGenerationP256() {
  ...
  return crypto.subtle.generateKey(
    ...
    assert.strictEqual(keyPair.publicKey.algorithm.name, 'ECDH', 'Algorithm name should be ECDH');
...
function testECDHKeyDerivation() {
  ...
  return Promise.all([alicePromise, bobPromise]).then(function(keyPairs) {
    ...
    assert.strictEqual(aliceSecret.length, bobSecret.length, 'Secrets should have same length');
    ...
    assert.strictEqual(secretsMatch, true, 'Both parties should derive the same secret');
    console.log('✅ Both parties successfully derived the same shared secret');
  }).catch(function(error) {
...
function testECDHDifferentCurves() {
  ...
  curves.forEach(function(curve) {
    const promise = Promise.all([
      ...
      assert.strictEqual(sharedSecret.byteLength, 32, 'Derived secret should be 32 bytes');
    }).catch(function(error) {
      console.error('❌ ECDH test failed for curve', curve + ':', error.message);
      throw error;
    });
...
function testECDHDeriveAESKey() {
  ...
  return Promise.all([
    ...
    assert.strictEqual(aesKey.type, 'secret', 'Derived key should be a secret key');
    assert.strictEqual(aesKey.algorithm.name, 'AES-GCM', 'Derived key algorithm should be AES-GCM');
    assert.strictEqual(aesKey.algorithm.length, 256, 'Derived key length should be 256 bits');
    
    console.log('✅ Derived AES key has correct properties');
  }).catch(function(error) {
    // Note: deriveKey might not be fully implemented yet
    if (error.name === 'NotSupportedError') {
      console.log('⚠️ deriveKey not yet implemented, testing deriveBits instead');
...
function testECDHInvalidParameters() {
  ...
  return crypto.subtle.generateKey(
    ...
    if (error.name === 'NotSupportedError' || error.message.includes('Unsupported')) {
