...
function testMultiPartyKeyExchange() {
  ...
  return Promise.all([alicePromise, bobPromise, charliePromise]).then(function(keyPairs) {
    ...
    return Promise.all([aliceBobPromise, bobCharliePromise, charlieAlicePromise]);
  }).then(function(secrets) {
    console.log('✅ All three pairwise secrets derived');
    console.log('   Alice-Bob secret length:', new Uint8Array(secrets[0]).length, 'bytes');
    console.log('   Bob-Charlie secret length:', new Uint8Array(secrets[1]).length, 'bytes');
    console.log('   Charlie-Alice secret length:', new Uint8Array(secrets[2]).length, 'bytes');
    ...
    assert.strictEqual(abBcDifferent, true, 'Alice-Bob and Bob-Charlie secrets should differ');
    assert.strictEqual(bcCaDifferent, true, 'Bob-Charlie and Charlie-Alice secrets should differ');
    assert.strictEqual(caAbDifferent, true, 'Charlie-Alice and Alice-Bob secrets should differ');
    
    console.log('✅ All pairwise secrets are unique');
  }).catch(function(error) {
    console.error('❌ Multi-party key exchange test failed:', error.message);
    throw error;
  });
}
...
function testDifferentBitLengths() {
  ...
  return Promise.all([
    ...
    bitLengths.forEach(function(bits) {
      derivePromises.push(
        crypto.subtle.deriveBits(
          ...
          console.log('✅ Derived', bits, 'bits (', bytes.length, 'bytes)');
          // P-256 might clamp to max 256 bits
          const expectedBytes = Math.min(bits, 256) / 8;
          assert.strictEqual(bytes.length, expectedBytes, 'Derived bytes length should match expected');
          return bytes;
        }).catch(function(error) {
          console.log('   Could not derive', bits, 'bits:', error.message);
          return null;
        })
      );
    });
    ...
    const validSecrets = secrets.filter(function(s) { return s !== null; });
    if (validSecrets.length > 0) {
      console.log('✅ Successfully derived', validSecrets.length, 'different bit lengths');
    }
  }).catch(function(error) {
...
function testCrossCurveDerivation() {
  ...
  return Promise.all([p256Promise, p384Promise]).then(function(keyPairs) {
    ...
    return crypto.subtle.deriveBits(
      ...
      console.log('✅ Cross-curve derivation correctly failed:', error.message);
    });
  }).catch(function(error) {
    if (error.message && error.message.includes('Expected error')) {
      throw error;
...
function testSelfDerivation() {
  ...
  return crypto.subtle.generateKey(
    ...
    const bytes = new Uint8Array(secret);
    console.log('✅ Self-derivation succeeded, secret length:', bytes.length, 'bytes');
    
    // The secret should still be valid (mathematically it's the private key times the generator point times the private key)
    assert.strictEqual(bytes.length, 32, 'Self-derived secret should be 32 bytes');
    ...
    assert.strictEqual(hasNonZero, true, 'Self-derived secret should not be all zeros');
...
function testMaximumBitLength() {
  ...
  Object.keys(curveMaxBits).forEach(function(curve) {
    ...
    const promise = Promise.all([
      ...
      const bytes = new Uint8Array(secret);
      console.log('✅', curve, ': Derived', maxBits, 'bits (', bytes.length, 'bytes)');
      assert.strictEqual(bytes.length * 8, maxBits, 'Max bits derivation for ' + curve);
    });
...
function testZeroBitsDerivation() {
  ...
  return Promise.all([
    ...
    return crypto.subtle.deriveBits(
      ...
      const bytes = new Uint8Array(secret);
      console.log('Zero bits derivation returned buffer of length:', bytes.length);
      assert.strictEqual(bytes.length, 0, 'Zero bits should return empty buffer');
      console.log('✅ Zero bits derivation handled correctly');
    }).catch(function(error) {
      // Some implementations might reject 0 bits
      console.log('✅ Zero bits derivation rejected (also acceptable):', error.message);
    });
  }).catch(function(error) {
...
function testDerivationConsistency() {
  ...
  return Promise.all([
    ...
    for (let i = 1; i < secrets.length; i++) {
      ...
      assert.strictEqual(currentSecret.length, firstSecret.length, 'Secret lengths should match');
      
      for (let j = 0; j < firstSecret.length; j++) {
        assert.strictEqual(currentSecret[j], firstSecret[j], 'Secret bytes should match at position ' + j);
      }
    }
    
    console.log('✅ All 5 derivations produced identical secrets (deterministic)');
  }).catch(function(error) {
...
function testInvalidPublicKey() {
  ...
  return crypto.subtle.generateKey(
    ...
    const allRejected = results.every(function(rejected) { return rejected; });
    assert.strictEqual(allRejected, true, 'All invalid keys should be rejected');
