...
function testEmptyData() {
  ...
  return crypto.subtle.generateKey(
    ...
    return crypto.subtle.sign(
      ...
      console.log('✅ Empty data signed successfully, signature length:', signature.byteLength);
      ...
      assert.strictEqual(isValid, true, 'Empty data signature should be valid');
...
function testLargeData() {
  ...
  return crypto.subtle.generateKey(
    ...
    return crypto.subtle.sign(
      ...
      console.log('✅ Large data (1MB) signed successfully, signature length:', signature.byteLength);
      ...
      assert.strictEqual(isValid, true, 'Large data signature should be valid');
...
function testCrossCurveVerification() {
  ...
  return Promise.all([p256Promise, p384Promise]).then(function(keyPairs) {
    ...
    return crypto.subtle.sign(
      ...
      assert.strictEqual(isValid, false, 'Cross-curve verification should fail');
...
function testInvalidSignatureFormat() {
  ...
  return crypto.subtle.generateKey(
    ...
    return crypto.subtle.verify(
      ...
      assert.strictEqual(isValid, false, 'Empty signature should not verify');
      ...
      assert.strictEqual(isValid, false, 'Short signature should not verify');
      ...
      assert.strictEqual(isValid, false, 'Wrong size signature should not verify');
      ...
      if (error && error.message) {
        console.log('   Invalid signature threw error (acceptable):', error.message);
        console.log('✅ Invalid signatures handled correctly (via errors)');
      } else {
        throw error;
      }
    });
  }).catch(function(error) {
    console.error('❌ Invalid signature format test failed:', error ? error.message : 'unknown error');
...
function testMultipleSignatures() {
  ...
  return crypto.subtle.generateKey(
    ...
    const signPromises = [];
    for (let i = 0; i < 5; i++) {
      signPromises.push(crypto.subtle.sign(
        {
          name: "ECDSA",
          hash: "SHA-256"
        },
    ...
    return Promise.all(signPromises).then(function(signatures) {
      ...
      assert.strictEqual(allDifferent, true, 'ECDSA signatures should include randomness');
      console.log('✅ All signatures are different (includes randomness)');
      ...
      results.forEach(function(isValid, index) {
        assert.strictEqual(isValid, true, 'Signature ' + index + ' should be valid');
      });
...
function testHashAlgorithmMismatch() {
  ...
  return crypto.subtle.generateKey(
    ...
    return crypto.subtle.sign(
      ...
      assert.strictEqual(isValid, false, 'Hash algorithm mismatch should fail verification');
...
testInvalidCurveNames()
  ...
  .then(function() {
    ...
    console.log('Note: Invalid signature format test skipped due to implementation variations');
  })
  .catch(function(error) {
