...
if (typeof crypto === 'undefined' || !crypto.subtle) {
  ...
  try {
    ...
    keyPromise
      .then((key) => {
        ...
        // Sign the data
        const signPromise = crypto.subtle.sign('HMAC', key, data);
        ...
        signPromise
          .then((signature) => {
            ...
            console.log(
              ...
              Array.from(signatureArray.slice(0, 8))
            );

            // HMAC-SHA-256 should produce 32-byte signature
            if (signatureArray.length === 32) {
              console.log(
                '✅ PASS: Signature length correct (32 bytes for HMAC-SHA-256)'
              );
            } else {
              console.log(
                '❌ FAIL: Signature length incorrect. Expected: 32, Got:',
            ...
            verifyPromise
              .then((isValid) => {
                ...
                if (isValid === true) {
                  console.log(
                    '✅ PASS: HMAC sign/verify round-trip successful!'
                  );
                  ...
                  console.log(
                    '❌ FAIL: Verification failed for valid signature'
                  );
                }
              })
              .catch((error) => {
      ...
      .catch((error) => {
        console.error(
          '❌ FAIL: HMAC key generation failed:',
          error.name,
          error.message
        );
      });
    ...
    console.error(
      '❌ FAIL: Exception in HMAC key generation:',
      error.name,
      error.message
    );
  }
  ...
  function testWrongData(key, originalSignature) {
    ...
    crypto.subtle
      .verify('HMAC', key, originalSignature, wrongData)
      .then((isValid) => {
        if (isValid === false) {
          console.log(
            '✅ PASS: Verification with wrong data failed as expected (authentication works!)'
          );
          ...
          console.log(
            '❌ FAIL: Verification with wrong data should have failed but succeeded'
          );
        }
      })
      .catch((error) => {
        console.error(
          '❌ FAIL: Verification with wrong data threw error:',
          error.name,
          error.message
        );
      });
      ...
      crypto.subtle
        ...
        .then((key) => {
          ...
          return crypto.subtle
            .sign('HMAC', key, testData)
            .then((signature) => {
              const signatureArray = new Uint8Array(signature);

              if (signatureArray.length === expectedSize) {
                console.log(
                  `✅ PASS: HMAC-${hashAlg} signature size correct (${expectedSize} bytes)`
                );
                ...
                console.log(
                  `❌ FAIL: HMAC-${hashAlg} signature size incorrect. Expected: ${expectedSize}, Got: ${signatureArray.length}`
                );
