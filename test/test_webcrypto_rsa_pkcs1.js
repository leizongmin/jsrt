...
function testRSAPKCS1v15KeyGeneration() {
  ...
  return crypto.subtle.generateKey(
    ...
    assert.strictEqual(keyPair.publicKey.algorithm.name, 'RSA-PKCS1-v1_5', 'Algorithm name should be RSA-PKCS1-v1_5');
...
function testRSAPKCS1v15Encryption(keyPair) {
  ...
  return crypto.subtle.encrypt(
    ...
    console.log('✓ RSA-PKCS1-v1_5 encryption successful, ciphertext length:', ciphertext.byteLength);
    ...
    console.log('✓ RSA-PKCS1-v1_5 decryption successful, decrypted text:', decryptedText);
    
    // Use assert for verification
    assert.strictEqual(decryptedText, "Hello, RSA-PKCS1-v1_5!", 'Decrypted text should match original plaintext');
    console.log('✓ RSA-PKCS1-v1_5 encryption/decryption test PASSED');
  }).catch(function(error) {
    console.error('✗ RSA-PKCS1-v1_5 encryption/decryption test FAILED:', error);
    throw error;
  });
}
...
function runTests() {
  ...
  testRSAPKCS1v15KeyGeneration().then(function(keyPair) {
