...
console.log('Test: Exported Global - i32 immutable');
{
  ...
  if (!(g instanceof WebAssembly.Global)) throw new Error('Not a Global object');
  if (g.value !== 42) throw new Error(`Expected 42, got ${g.value}`);
  if (g.valueOf() !== 42) throw new Error(`valueOf: Expected 42, got ${g.valueOf()}`);
...
console.log('Test: Exported Global - i32 mutable');
{
  ...
  if (counter.value !== 0) throw new Error(`Initial: Expected 0, got ${counter.value}`);

  counter.value = 42;
  if (counter.value !== 42) throw new Error(`After set: Expected 42, got ${counter.value}`);

  counter.value = -100;
  if (counter.value !== -100) throw new Error(`After set: Expected -100, got ${counter.value}`);
